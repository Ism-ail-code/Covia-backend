-- =============================================================
-- 0036: Fix search_rides / get_ride PostgREST compatibility
-- =============================================================
-- Found live (2026-08-02 E2E): the functions were declared
-- `stable` yet call expire_overdue_rides(), which writes. PostgREST
-- executes STABLE/IMMUTABLE RPCs inside read-only transactions, so
-- every call raised 25006 "cannot execute INSERT in a read-only
-- transaction". The 0013-era 12-parameter search_rides overload was
-- also never dropped, leaving two overloads that PostgREST cannot
-- resolve (PGRST102 on any parameterised body).
--
-- Fix: drop the legacy overload and recreate both read functions as
-- VOLATILE (they legitimately purge expired rides as a side effect).
-- Clients call them via POST (supabase-js rpc()), so the volatility
-- change only affects PostgREST's transaction mode.

-- ── 1. Drop the legacy 12-parameter search_rides overload (0013) ──
drop function if exists public.search_rides(
  text, text, date, time, integer, boolean, boolean, text, numeric, numeric, integer, integer
);

-- ── 2. search_rides → volatile ──
